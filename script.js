...
function checkPose(prediction, video) {
    ...
    if (!poseStates[`pose${poseNumber}`]) {
     ...
     if (prob > 0.8 && !explosionActive) {
        ...
        switch(poseNumber) {
            case '1':
                if (time >= 4 && time <= 6 && !poseState.triggered) {
                    triggerExplosion(poseState);
                } //0.2,2.5,4,7.5,8,9,15,16,18.5
                break;
            case '2':
                if (time >= 6.1 && time <= 8 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '3':
                if (time >= 11 && time <= 13.5 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '4':
                if (time >=19  && time <= 22.9 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '5':
                if (time >= 23 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
        }
